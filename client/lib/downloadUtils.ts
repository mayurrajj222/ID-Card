...
export const downloadAsImage = async (elementId: string, filename: string = 'student-id-card') => {
  try {
...
export const downloadAsPDF = async (elementId: string, filename: string = 'student-id-card') => {
  try {
    const element = document.getElementById(elementId);
    if (!element) {
      throw new Error('Element not found');
